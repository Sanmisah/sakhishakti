...
<div>
    ...
    <div class="pt-5">        
        <form class="space-y-5" action="{{ route('medical_assessments.create') }}" method="POST">
            @csrf
            <div class="panel">
                <div class="grid grid-cols-1 gap-4 mb-4 md:grid-cols-4">
                    <input type="hidden" name="visitor_code" value="{{ $visitor_code }}">
                    <input type="hidden" name="visitor_id" value="{{ $visitors->id }}">
                    <x-text-input name="date" class="bg-gray-100 dark:bg-gray-700" readonly="true" value="{{ $visitors->created_at->format('d/m/Y') }}" :label="__('Date')" :messages="$errors->get('created_at')"/>
